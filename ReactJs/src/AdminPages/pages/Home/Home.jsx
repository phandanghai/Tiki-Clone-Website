...
const Home = () => {
  ...
  useEffect(() => {
    ApiGetAllProduct(dispatch).then((data) => {
      ...
      const dataFilter = Object.entries(filteredData).map(
        ([category, count]) => ({
          name: category,
          value: count,
        })
  ...
  useEffect(() => {
    ApiGetAllOrder(dispatch).then((data) => {
      ...
      data?.data?.orders?.forEach((item, index) => {
        if (item.status_order === "Thành công") {
          console.log(0);
          filteredData[monthNames[index]] =
            filteredData[monthNames[index]] +
            item?.price_product * item.number_order;
        }
      });
      const dataFilter = Object.entries(filteredData).map(([month, count]) => ({
        name: month,
