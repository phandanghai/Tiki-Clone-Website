  ...
  useEffect(() => {
    ...
    if (id) {
      ApiGetProduct({ id_product: id }).then((data) => {
        ...
        setPreviews([
          data?.product.image,
          data?.product.image_2,
          data?.product.image_3,
          data?.product.image_4,
          data?.product.image_5,
        ]);
        setListBase64([
          data?.product.image,
          data?.product.image_2,
  ...
  return (
    <div className="w-full h-[550px] bg-main-bg grid grid-cols-10 gap-4 p-8 text-white overflow-y-auto">
      <div className="col-span-7 p-4">
        ...
        <div className="w-full mt-4 p-6 flex flex-col gap-3 bg-[#64748b] h-max rounded-lg">
          ...
          <div className="flex flex-col gap-2 w-full h-max">
            ...
            <textarea
              ...
              onChange={(e) =>
                setProduct({ ...product, description_product: e.target.value })
              }
            ></textarea>
          </div>
        </div>
        ...
        <div className="w-full mt-4 p-6 flex flex-col gap-3 bg-[#64748b] h-max rounded-lg">
          <h2 className="text-16 text-white">
            Thông tin về giá và nhà cung cấp
          </h2>
          <div className="gap-3 grid grid-cols-2 grid-rows-2">
            ...
            <div className="flex flex-col gap-2 w-full h-max">
              ...
              <input
                type="text"
                value={Math.round(
                  (product?.price_product * (100 - product?.discount)) / 100
                )}
                className="w-full outline-none pl-5 h-9 rounded-[4px] bg-[#546277]"
              />
            </div>
      ...
      <div className="col-span-3 rounded-lg py-6 px-4">
          ...
          {[0, 1, 2, 3, 4].map((item) => {
            return (
              ...
              >
                ...
                <input
                  type="file"
                  onChange={(e) => handleChooseImages(e, item)}
                  className="hidden"
                  ref={inputRef}
                />
              </div>
        ...
        {window.location.href.split("/")[6] ? (
          <button
            className="mt-8 ml-3 w-[180px] h-9 rounded-[8px] bg-[#64748b]"
