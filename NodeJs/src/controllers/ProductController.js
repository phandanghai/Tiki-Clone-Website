...
const ProductController = {
   ...
   getBrand: async (req, res) => {
      try {
         ...
         const data = result[0].map((item) => item.brand_product).filter((brand) => brand !== null);
         return res.status(200).json({
   ...
   getStores: async (req, res) => {
      try {
         ...
         const data = result[0].map((item) => item.stores_product).filter((store) => store !== null);
         return res.status(200).json({
   ...
   getDataFilter: async (req, res) => {
      try {
         const { service_product, stars, maxPrice, minPrice, brand_product, stores_product } = req.body.filter;
         const result = await ProductsModels.getDataFilterModels({ service_product, stars, maxPrice, minPrice, brand_product, stores_product });
         return res.status(200).json({
