...
const UserController = {
   ...
   createUserAdmin: async (req, res) => {
      try {
         const { full_name, username, birthday, email, avatar, password, phone, sex, verified } = req.body;
         const saltRounds = 10;
   ...
   register: async (req, res) => {
      try {
         ...
         if (hashedPassword) {
            ...
            const result = await UserModels.registerModels({ id_user, email, password: hashedPassword, full_name: fullname, username: username });
            if (result) {
               return res.status(200).json({ message: 'Registration successful', user: result });
            } else {
               return res.status(400).json({ message: 'Registration failed' });
   ...
   uploadAvatar: async (req, res) => {
      try {
         ...
         if (req.body.base64) {
            const listBase64 = req.body.base64.filter((item) => item.startsWith('data'));
            ...
            return res.status(200).json({ message: 'Not Avatar Images', image: null });
         }
      } catch (error) {
         console.log(error);
      }
   ...
   updateUser: async (req, res) => {
      try {
         const { id_user, full_name, username, address, countries, sex, verified, avatar, phone, email, birthday } = req.body;
   ...
   getListAddresss: async (req, res) => {
      try {
         const result = await UserModels.getListAddressModels({ id_user: req.body.id_user });
         if (result) {
            return res.status(200).json({
   ...
   createNewAddress: async (req, res) => {
      try {
         const { id_user, customer, company, phone, province, district, ward, address, type_address, default_address } = req.body.address;
         console.log(id_user);
         const getIdAddress = await UserModels.getListAddressModels({ id_user: id_user });
         console.log(getIdAddress);
         if (getIdAddress[0]?.address_1 === null) {
            ...
            if (default_address) {
               const result2 = await UserModels.setDefaultAddressModels({ id_user, id: 1 });
            }
            return res.status(200).json({
               message: 'Create NewAddress successfully',
            });
         } else if (getIdAddress[0]?.address_1 && getIdAddress[0]?.address_2 === null) {
            const result1 = await UserModels.createNewAddressModels({
               id_user,
               customer,
               company,
            ...
            if (default_address) {
               const result2 = await UserModels.setDefaultAddressModels({ id_user, id: 2 });
            }
            return res.status(200).json({
               message: 'Create NewAddress successfully',
            });
            ...
            if (default_address) {
               const result2 = await UserModels.setDefaultAddressModels({ id_user, id: 3 });
            }
            return res.status(200).json({
               message: 'Create NewAddress successfully',
            });
   ...
   setDefaultAddress: async (req, res) => {
      try {
         ...
         const result = UserModels.setDefaultAddressModels({ id_user: id_user, id: id });
         if (result) {
            return res.status(200).json({
   ...
   updatePassword: async (req, res) => {
      try {
         ...
         if (!isPass) {
            ...
            const result = await UserModels.updatePasswordModels({ id_user, newPassword: hashedPassword });
            if (result) {
               const user = await UserModels.getUserModels({ id_user: id_user });
               return res.status(200).json({
   ...
   updatePasswordByEmail: async (req, res) => {
      try {
         ...
         const saltRounds = 10;
         const hashedPassword = await bcrypt.hash(newPassword, saltRounds);
         const result = await UserModels.updatePasswordByEmailModels({ email, passwordCode: hashedPassword });
         if (result) {
            return res.status(200).json({
