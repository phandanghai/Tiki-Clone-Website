...
const ProductsModels = {
   ...
   getProductModels: async ({ id_product }) => {
      const result = await database.query(`SELECT * FROM Products WHERE id_product =?`, [id_product]);
      return result[0][0];
   },
   ...
   deleteProductModels: async ({ id_product }) => {
      const result = await database.query(`DELETE FROM Products WHERE id_product =?`, [id_product]);
      return result;
   },
   ...
   getBrandModels: async () => {
      const result = await database.query(`SELECT DISTINCT brand_product FROM Products`);
      return result;
   },
   getStoresModels: async () => {
      const result = await database.query(`SELECT DISTINCT stores_product FROM Products`);
      return result;
   },
   getDataFilterModels: async ({ service_product, stars, maxPrice, minPrice, brand_product, stores_product }) => {
      let queryStores;
      if (stores_product.length > 0) {
         queryStores = `AND stores_product IN (${stores_product.map((store) => `"${store}"`).join(', ')})`;
      } else {
         queryStores = ``;
      }
      let queryBrand;
      if (brand_product.length > 0) {
         queryBrand = `AND brand_product IN (${brand_product.map((brand) => `"${brand}"`).join(', ')})`;
      } else {
         queryBrand = ``;
      }
      let queryMinPrice;
